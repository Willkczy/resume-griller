...
class GapAnalysis:
    ...
    severity: float = 0.0
    priority_gap: GapType | None = None
...
class GrillingEngine:
    ...
    async def evaluate_answer(
        ...
        if (
            self.FORCE_FIRST_FOLLOWUP
            and follow_up_count < self.MIN_FOLLOWUPS_PER_QUESTION
        ):
                ...
                if not evaluation.suggested_follow_up:
                    if self.use_compact_prompts:
                        evaluation.suggested_follow_up = (
                            await self._generate_followup_compact(
                                question,
                                answer,
                        ...
                        evaluation.suggested_follow_up = (
                            await self._generate_gap_based_followup(
...
Valid gaps: no_specific_example, no_metrics, too_generic, unclear_role, no_outcome, no_tech_depth"""
    ...
    def _parse_compact_evaluation(
        self, response: str, follow_up_count: int
    ) -> AnswerEvaluation:
        ...
        try:
            ...
            if json_match:
                ...
                is_sufficient = data.get(
                    "sufficient", score >= self.SUFFICIENT_SCORE_THRESHOLD
                )
...
Follow-up:"""
        ...
        try:
            ...
            return FOLLOWUP_TEMPLATES.get(
                priority_gap, ["Could you provide more specific details?"]
            )[0]

    def _create_short_answer_evaluation(
        self, answer: str, follow_up_count: int
    ) -> AnswerEvaluation:
...
no_metrics, unclear_role, no_outcome, no_tech_depth, solution_not_exp, vague_ambiguous"""
    ...
    def _parse_llm_evaluation(
        ...
        try:
            ...
            is_sufficient = (
                overall_score >= self.SUFFICIENT_SCORE_THRESHOLD
                and data.get("is_sufficient", False)
                and len(detected_gaps) <= 2
            )
    ...
    def _fallback_evaluation(
        ...
        has_specificity = sum(
            1 for ind in specificity_indicators if ind in answer_lower
        )
        ...
        priority_gap = (
            detected_gaps[0] if detected_gaps else GapType.NO_SPECIFIC_EXAMPLE
        )

        return AnswerEvaluation(
            is_sufficient=base_score >= self.SUFFICIENT_SCORE_THRESHOLD
            ...
            suggested_follow_up=FOLLOWUP_TEMPLATES.get(
                priority_gap, ["Could you elaborate?"]
            )[0],
